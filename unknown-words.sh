    ...
    push)
      if [ -n "$INPUT_SUPPRESS_PUSH_FOR_OPEN_PULL_REQUEST" ]; then
        ...
        if [ -n "$(jq .documentation_url $pull_request_json 2>/dev/null)" ]; then
          (
            echo 'The object shape is an error, it is probably an authentication error.'
            if [ -n "$ACT" ]; then
      ...
      issue_comment|pull_request|pull_request_target)
        if [ ! -d $bucket/$project ]; then
          ...
          if [ -z "$pull_request_url" ]; then
            ...
            pull_request_sha=$(jq -r ".sha $pull_request_info // empty")
...
repo_is_private() {
  private=$(jq -r '.repository.private // empty' "$GITHUB_EVENT_PATH")
  [ "$private" != "false" ]
}
